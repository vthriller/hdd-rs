...
use scsi::{SCSICommon, SCSIDevice};
...
impl<'a> SCSIPages<'a, SCSIDevice> {
    ...
    fn get_params(&mut self, page: u8) -> Result<Vec<log_page::Parameter>, Error> {
        ...
        page.parse_params()
            .ok_or(Error::InvalidData("parse log page params"))
    }
        ...
        let counters = params
            ...
            .map(|param| {
                ...
                let value = {
                    ...
                    (&param.value[offset..])
                        .read_uint::<BigEndian>(param.value.len() - offset)
                        .unwrap()
                };
    ...
    pub fn non_medium_error_count(&mut self) -> Result<u64, Error> {
        ...
        for param in params {
            ...
            return Ok((&param.value[..])
                .read_uint::<BigEndian>(param.value.len())
                .unwrap());
        }
    ...
    */
        ...
        for param in params {
            match param.code {
                ...
                0x0003 => {
                    ...
                    result.lifetime_start_stop_cycles =
                        Some((&param.value[0..4]).read_u32::<BigEndian>().unwrap());
                }
                0x0004 => {
                    // XXX tell about unexpected params?
                    if param.value.len() < 4 {
                ...
                0x0005 => {
                    ...
                    result.lifetime_load_unload_cycles =
                        Some((&param.value[0..4]).read_u32::<BigEndian>().unwrap());
                }
                0x0006 => {
                    // XXX tell about unexpected params?
    ...
    pub fn self_test_results(&mut self) -> Result<Vec<SelfTest>, Error> {
        ...
        let self_tests = params
            .iter()
            .map(|param| {
                // XXX tell about unexpected params?
                if param.code == 0 || param.code > 0x0014 {
                    return None;
                }
                if param.value.len() < 0x10 {
                    return None;
                }

                // unused self-test log parameter is all zeroes
                if *param.value.iter().max().unwrap() == 0 {
                    return None;
                }
