...
impl Error {
    fn from_sense(sense: &sense::Sense) -> Self {
        match sense.kcq() {
            Some((key, asc, ascq)) => Error::Sense(sense::key::SenseKey::from(key), asc, ascq),
            None => Error::Nonsense,
        }
    }
}
...
pub trait SCSICommon: Sized {
    ...
    */
    ...
    fn ata_pass_through_16(
        &self,
        dir: Direction,
        regs: &ata::RegistersWrite,
    ) -> Result<(ata::RegistersRead, Vec<u8>), ATAError> {
        info!(
            "issuing ATA PASS-THROUGH (16): dir={:?} regs={:?}",
            dir, regs
        );
...
where
    C: ::std::ops::Div<Output = C> + ::std::convert::From<u8> + ::std::fmt::Display,
{
    ...
    info!(
        "issuing READ DEFECT DATA({}): plist={:?} glist={:?} format={:?}",
        log_detail, plist, glist, format
    );
    ...
    if sense.len() > 0 {
        ...
        if let Some((true, sense)) = sense::parse(&sense) {
            match sense.kcq() {
				...
				// unexpected sense
				_s => return Err(Error::from_sense(&sense)),
			}
        }
    }
    ...
    if let Some((format, glistv, plistv, len)) = parse(&data) {
        debug!(
            "defect list: format={} glistv={} plistv={} len={}\n",
